...
% Source: 2.6 Vallado, David A. Fundamentals of Astrodynamics and Applications (2013)
% Ω: RAAN
% ν: true anomaly
% λ: true longitude
...
% Assume Circular Inclined
% argumentPerigee = 0.0;
% u = argumentPerigee + 
% trueAnomaly     = u;
% 
% % Position and velocity in the perifocal coordinate system
% p = semi_major_axis*(1 - eccentricity^2); % Semiparameter
% rP = p*cos(true_anomaly) / (1 + eccentricity*cos(true_anomaly));
% rQ = p*sin(true_anomaly) / (1 + eccentricity*cos(true_anomaly));
% rW = 0;
% 
% rPQW = [rP; 
%         rQ; 
%         rW];
% 
% vP = -sqrt(muEarth/p) * sin(true_anomaly);
% vQ = sqrt(muEarth/p) * (eccentricity + cos(true_anomaly));
% vW = 0;
% 
% vPQW = [vP;
%         vQ;
